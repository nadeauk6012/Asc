...
static bool enabled, LevelEndGameBoost, DungeonsOnly, PlayerChangeNotify, LevelUseDb, rewardEnabled, DungeonScaleDownXP, DungeonScaleDownMoney, CountNpcBots, ModifySpecificMapsOnly;
...
static std::set<uint32> specificMapIds; // New set to hold specific map IDs
...
void LoadSpecificMapIds(std::string mapIdString) // Used for reading the string from the configuration file for selecting specific map IDs
{
    std::string delimitedValue;
    std::stringstream mapIdStream;

    mapIdStream.str(mapIdString);
    while (std::getline(mapIdStream, delimitedValue, ',')) // Process each map ID in the string, delimited by the comma - ","
    {
        uint32 mapId = atoi(delimitedValue.c_str());
        if (mapId > 0)
        {
            specificMapIds.insert(mapId);
        }
    }
}
...
bool isSpecificMap(uint32 mapId)
{
    return (specificMapIds.find(mapId) != specificMapIds.end());
}
...
public:
    ...
    void SetInitialWorldSettings()
    {
        ...
        bossOverrides.clear();
        specificMapIds.clear(); // Clear specificMapIds set
        ...
        LoadSpecificMapIds(sConfigMgr->GetOption<std::string>("AutoBalance.SpecificMapIds", "")); // Load specific map IDs
        ...
        //end npcbot
        ModifySpecificMapsOnly = sConfigMgr->GetOption<bool>("AutoBalance.ModifySpecificMapsOnly", 0); // Load ModifySpecificMapsOnly option
...
public:
    ...
    void OnPlayerEnterAll(Map* map, Player* player)
    {
        ...
        if (ModifySpecificMapsOnly && !isSpecificMap(map->GetId())) // Check if ModifySpecificMapsOnly is enabled and if the map is in the specificMapIds set
            return;
...
public:
    ...
    void Creature_SelectLevel(const CreatureTemplate* /*creatureTemplate*/, Creature* creature) override
    {
        ...
        if (ModifySpecificMapsOnly && !isSpecificMap(creature->GetMap()->GetId())) // Check if ModifySpecificMapsOnly is enabled and if the map is in the specificMapIds set
            return;
