        ...
        if (oldItem)
        {
            ...
            if (sT->GetUseCollectionSystem())
            {
                ...
                if (sT->collectionCache.find(accountId) != sT->collectionCache.end())
                {
                    ...
                    //Dinkle
                    std::set<uint32> uniqueDisplayIds;
                    //endDinkle
                    ...
                    for (uint32 newItemEntryId : sT->collectionCache[accountId]) {
                        ...
                        if (hasSearchString && newItem->GetTemplate()->Name1.find(searchDisplayValue) == std::string::npos)
                            continue;
                        if (!uniqueDisplayIds.insert(newItem->GetTemplate()->DisplayInfoID).second)
                            continue;
                        allowedItems.push_back(newItem);
                    }
                    std::sort(allowedItems.begin(), allowedItems.end(), [](Item* a, Item* b) {
                        return a->GetTemplate()->Name1 < b->GetTemplate()->Name1;
                        });
