...
public:
    ...
    void OnBeforeChooseGraveyard(Player* player, TeamId /*teamId*/, bool /*nearCorpse*/, uint32& graveyardOverride) override
    {
        ...
        {
            graveyardOverride = 2909;
        }
    }

    void OnPlayerReleasedGhost(Player* player) override
    {
        if (player->GetMapId() == MAP_NAXX)
        {
            // Teleport player to their corpse location instead of the graveyard
            Corpse* corpse = player->GetCorpse();
            if (corpse)
            {
                player->TeleportTo(corpse->GetMapId(), corpse->GetPositionX(), corpse->GetPositionY(), corpse->GetPositionZ(), corpse->GetOrientation());
            }
        }
    }
};
