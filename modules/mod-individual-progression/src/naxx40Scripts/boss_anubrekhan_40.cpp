...
public:
    ...
    struct boss_anubrekhan_40AI : public BossAI
    {
        ...
        {
            if (Creature* guard1 = me->SummonCreature(NPC_CRYPT_GUARD, 3299.732f, -3502.489f, 287.077f, 2.378f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000))
            {
                if (me->IsInCombat())
                    guard1->SetInCombatWithZone();
            }
            if (Creature* guard2 = me->SummonCreature(NPC_CRYPT_GUARD, 3299.086f, -3450.929f, 287.077f, 3.999f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000))
            {
                if (me->IsInCombat())
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            events.ScheduleEvent(EVENT_LOCUST_SWARM, urand(55000, 70000));
            events.ScheduleEvent(EVENT_BERSERK, 600000);
            if (!summons.HasEntry(NPC_CRYPT_GUARD))
            {
                SummonCryptGuards();
            }
            // Set the initial Crypt Guards in combat
            std::list<Creature*> cryptGuards;
            GetCreatureListWithEntryInGrid(cryptGuards, me, NPC_CRYPT_GUARD, 80.0f);
            for (Creature* guard : cryptGuards)
            {
