        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case EVENT_FROST_BLAST:
                    ...
                    {
                        // Skip if target is within 8 yards of Kel'Thuzad
                ...
                case EVENT_DETONATE_MANA:
                {
                    ...
                    {
                        if ((itr->getTarget()->GetTypeId() == TYPEID_PLAYER || (itr->getTarget()->GetTypeId() == TYPEID_UNIT && static_cast<Creature*>(itr->getTarget())->IsNPCBot()))
                            && itr->getTarget()->getPowerType() == POWER_MANA
                            && itr->getTarget()->GetPower(POWER_MANA)
                            && me->GetDistance(itr->getTarget()) > 8.0f)
                        {
...
class spell_kelthuzad_dark_blast : public SpellScript
{
    ...
    void CalculateDamage(SpellEffIndex /*effIndex*/)
    {
        ...
        if (target->GetLevel() >= 60 && target->GetLevel() <= 63)
        {
...
class spell_kelthuzad_frost_blast40_custom : public SpellScriptLoader
{
public:
    spell_kelthuzad_frost_blast40_custom() : SpellScriptLoader("spell_kelthuzad_frost_blast40_custom") { }

    class spell_kelthuzad_frost_blast40_custom_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_kelthuzad_frost_blast40_custom_SpellScript);

        void CalculateDamage(SpellEffIndex /*effIndex*/)
        {
            if (Unit* target = GetHitUnit())
            {
                // Calculate 21% of target's max health
                int32 damage = CalculatePct(target->GetMaxHealth(), 21);
                SetHitDamage(damage);
            }
        }
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_kelthuzad_frost_blast40_custom_SpellScript::CalculateDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_kelthuzad_frost_blast40_custom_SpellScript();
    }
};
...
void AddSC_boss_kelthuzad_40()
{
    ...
    new spell_kelthuzad_frost_blast40_custom();
