...
const Position PosWrap[8] =
{
    ...
    {3462.90625f, -3872.73828125f, 300.81896972656f, 5.6570415496826f},
    {3456.1291503906f, -3914.5185546875f, 301.10934448242f, 0.12423393130302f},
    {3477.3161621094f, -3939.2448730469f, 303.53775024414f, 0.12423393130302f},
    {3458.7822265625f, -3878.0593261719f, 301.08111572266f, 5.6660332679749f},
    {3448.4306640625f, -3908.9865722656f, 304.53509521484f, 6.1005940437317f}
};
...
public:
    ...
    struct boss_maexxna_40AI : public BossAI
    {
        ...
        void RemoveAllWebWrapAuras()
        {
            std::list<Unit*> units;
            Acore::AnyUnitInObjectRangeCheck check(me, 150.0f);
            Acore::UnitListSearcher<Acore::AnyUnitInObjectRangeCheck> searcher(me, units, check);
            Cell::VisitAllObjects(me, searcher, 150.0f);

            for (Unit* unit : units)
            {
                if (unit->HasAura(SPELL_WEB_WRAP))
                {
                    unit->RemoveAurasDueToSpell(SPELL_WEB_WRAP);
                }
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_MODIFY_WEB_SPRAY_AURA:
            {
                ...
                for (Unit* unit : units)
                {
                    if (unit->IsAlive() && unit->HasAura(SPELL_WEB_SPRAY))
                    {
                        if (Aura* aura = unit->GetAura(SPELL_WEB_SPRAY))
                        {
            ...
            {
                int32 modifiedPoisonShockDamage = urand(1550, 1850);  // Set damage to a random value between 1550 and 1850
            ...
            case EVENT_WEB_WRAP:
                ...
                for (uint8 i = 0; i < 2; ++i)
                {
                    if (Unit* target = SelectRandomPlayerOrNPCBot(150.0f))
                    {
    ...
    struct boss_maexxna_webwrap40AI : public NullCreatureAI
    {
        ...
        void SetGUID(ObjectGuid guid, int32  /*param*/) override
        {
            ...
            if (victimGUID)
            {
                ...
                {
                    if ((victim->GetTypeId() == TYPEID_PLAYER || (victim->GetTypeId() == TYPEID_UNIT && victim->ToCreature()->IsNPCBot())) && victim->GetEntry() != NPC_WEB_WRAP)
                    {
                        int32 modifiedBaseDamage = urand(350, 450);
