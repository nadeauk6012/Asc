    ...
    struct boss_thaddius_40AI : public BossAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_THADDIUS_ENTER_COMBAT:
            {
                ...
                // Teleport NPCBots and pets to Thaddius' location
                std::list<Creature*> nearbyCreatures;
                Acore::AnyUnitInObjectRangeCheck botOrPetCheck(me, 180.0f);
                Acore::CreatureListSearcher<Acore::AnyUnitInObjectRangeCheck> botOrPetSearcher(me, nearbyCreatures, botOrPetCheck);
                Cell::VisitGridObjects(me, botOrPetSearcher, 180.0f);
                for (Creature* nearbyCreature : nearbyCreatures)
                {
                    if (nearbyCreature->IsNPCBotOrPet())
                    {
                        nearbyCreature->NearTeleportTo(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation());
                    }
                }
...
{
public:
    spell_thaddius_pos_neg_charge_40() : SpellScriptLoader("spell_thaddius_pos_neg_charge_40") { }
    ...
    class spell_thaddius_pos_neg_charge_40_SpellScript : public SpellScript
    {
        ...
        void HandleDamage(SpellEffIndex /*effIndex*/)
        {
            ...
            uint32 oppositeAura = 0;
            uint32 sameAuraStack = 0;

            // Determine the opposite and stacking auras
            if (GetTriggeringSpell()->Id == 28059) // Positive charge
            {
                oppositeAura = 28084; // Negative charge
                sameAuraStack = 29659; // Positive charge stack
            }
            else if (GetTriggeringSpell()->Id == 28084) // Negative charge
            {
                oppositeAura = 28059; // Positive charge
                sameAuraStack = 29660; // Negative charge stack
            }

            bool oppositeAuraFound = false;
            bool sameAuraFound = false;
            std::list<Unit*> unitList;

            // Search for units within 10 yards
            Acore::AnyUnitInObjectRangeCheck u_check(caster, 10.0f);
            Acore::UnitListSearcher<Acore::AnyUnitInObjectRangeCheck> searcher(caster, unitList, u_check);
            Cell::VisitAllObjects(caster, searcher, 10.0f);

            for (Unit* unit : unitList)
            {
                if (unit->HasAura(oppositeAura))
                {
                    oppositeAuraFound = true;
                }
                if (unit->HasAura(GetTriggeringSpell()->Id))
                {
                    sameAuraFound = true;
                    // If another unit with the same aura is within 5 yards, apply the stacking aura
                    if (unit->IsWithinDistInMap(caster, 5.0f))
                    {
                        caster->CastSpell(caster, sameAuraStack, true);
                        unit->CastSpell(unit, sameAuraStack, true);
                    }
                }
            }

            if (!oppositeAuraFound)
            {
                SetHitDamage(0); // No damage if no opposite aura found within range
            }
            else
            {
                // Adjust damage to 1500 for naxx40 heroic
                if (target->GetMap()->GetDifficulty() == RAID_DIFFICULTY_10MAN_HEROIC)
                {
                    SetHitDamage(1500);
                }
                // If the target is an NPC bot, apply the damage
                else if (target->GetTypeId() == TYPEID_UNIT && target->ToCreature() && target->ToCreature()->IsNPCBot())
                {
                    SetHitDamage(1500);
                }
                // Otherwise, apply the standard damage
                else
                {
                    SetHitDamage(4500);
                }
...
public:
    ...
    class spell_thaddius_polarity_shift_SpellScript : public SpellScript
    {
        ...
        void MoveBotsToPosition(Unit* caster)
        {
            if (!caster)
                return;
            ...
            Acore::UnitListSearcher<Acore::AnyUnitInObjectRangeCheck> searcher(caster, units, u_check);
            Cell::VisitAllObjects(caster, searcher, radius);

            // Filter for NPC bots and move them to the correct position
            for (Unit* unit : units)
            {
                if (unit->GetTypeId() == TYPEID_UNIT)
                {
                    ...
                    if (creature && creature->IsNPCBot() && creature->IsAlive())
                    {
                        ...
                        if (creature->HasAura(SPELL_POSITIVE_POLARITY))
                        {
                            targetPosition = positivePos;
                        }
                        else if (creature->HasAura(SPELL_NEGATIVE_POLARITY))
                        {
                            targetPosition = negativePos;
                        }
                        else
                        {
                            continue;
                        ...
                        creature->GetBotAI()->MoveToSendPosition(targetPosition);
                    }
                }
            }
        }
