...
public:
    ...
    struct boss_grobbulus_40AI : public BossAI
    {
        ...
        {
            if (spellInfo->Id == SPELL_SLIME_SPRAY && (target->GetTypeId() == TYPEID_PLAYER || target->IsNPCBot()))
            {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_MUTATING_INJECTION:
                CastSpellOnRandomTarget(SPELL_MUTATING_INJECTION, 150.0f);
                events.RepeatEvent(22000); // Schedule the next injection to occur after 22 seconds
                break;
            }
...
class spell_grobbulus_poison_40 : public SpellScriptLoader
{
public:
    spell_grobbulus_poison_40() : SpellScriptLoader("spell_grobbulus_poison_40") { }

    class spell_grobbulus_poison_40_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_grobbulus_poison_40_SpellScript);

        void FilterTargets(std::list<WorldObject*>& targets)
        {
            targets.remove_if([this](WorldObject* target) -> bool
                {
                    // Check if the target is an NPC bot or pet and exclude them
                    if (Creature* creature = target->ToCreature())
                    {
                        if (creature->IsNPCBotOrPet())
                            return true;
                    }
                    // Exclude targets outside of zero distance (ensures the logic is similar to previous filter)
                    return !GetCaster()->IsWithinDist3d(target, 0.0f);
                });
        }

        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_grobbulus_poison_40_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_grobbulus_poison_40_SpellScript();
    }
};
