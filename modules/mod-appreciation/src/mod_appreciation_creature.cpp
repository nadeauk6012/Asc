...
public:
    ...
    bool HasForbiddenItemOrAura(Player* player)
    {
        // Dinkle
        std::vector<uint32> forbiddenItems = { 800050, 800049, 800048, 90000 };
        uint32 forbiddenAura = 80089;

        // Dinkle
        for (uint32 itemId : forbiddenItems)
        {
        ...
        // Dinkle
        if (player->HasAura(forbiddenAura))
            return true;
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        // Dinkle: Check for challenge modes
        if (HasForbiddenItemOrAura(player))
        {
            player->GetSession()->SendNotification("You cannot use this during a challenge mode.");
            return true;
        }
